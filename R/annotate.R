...
#' @title distance2nearestFeature
#' @description distance2nearestFeature
#' @param g.idh
#' @param tss
#' @param full.output
#' @export
#' @examples
#' distance2nearestFeature(g.idh = g.idh, tss = tss, full.output = FALSE)
distance2nearestFeature <- function(g.idh,tss, full.output = FALSE)
{
  #require(GenomicRanges)
  ...
  ## if strand is +, and end is LESS than start -> change SIGN
  ...
  ## if strand is -, and end is MORE than start -> change SIGN
  met.tss[met.tss$end>met.tss$start.y & met.tss$strand.y=="-",dist.col] =
      -1* met.tss[met.tss$end>met.tss$start.y & met.tss$strand.y=="-",dist.col]
  ...
  if(full.output) return(list(res = res, met.tss = met.tss))
  return(res)
}
